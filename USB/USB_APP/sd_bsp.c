...
static SD_Error SD_EnableWideBusOperation(u32 wmode);
...
/*用于sdio初始化的结构体*/
SDIO_InitTypeDef USB_SDIO_InitStructure;
SDIO_CmdInitTypeDef USB_SDIO_CmdInitStructure;
SDIO_DataInitTypeDef USB_SDIO_DataInitStructure;   
...
static volatile u8 TransferEnd=0;								//传输结束标志,DMA读写时使用
...
static SD_Error SD_PowerON(void)
{
	...
	u32 SDType=SD_STD_CAPACITY;
	 ...
	 /*初始化时的时钟不能大于400KHz*/ 
  USB_SDIO_InitStructure.SDIO_ClockDiv = SDIO_INIT_CLK_DIV;	/* HCLK = 72MHz, SDIOCLK = 72MHz, SDIO_CK = HCLK/(178 + 2) = 400 KHz */
  USB_SDIO_InitStructure.SDIO_ClockEdge = SDIO_ClockEdge_Rising;
  USB_SDIO_InitStructure.SDIO_ClockBypass = SDIO_ClockBypass_Disable;  //不使用bypass模式，直接用HCLK进行分频得到SDIO_CK
  USB_SDIO_InitStructure.SDIO_ClockPowerSave = SDIO_ClockPowerSave_Disable;	// 空闲时不关闭时钟电源
  USB_SDIO_InitStructure.SDIO_BusWide = SDIO_BusWide_1b;	 				//1位数据线
  USB_SDIO_InitStructure.SDIO_HardwareFlowControl = SDIO_HardwareFlowControl_Disable;//硬件流
  SDIO_Init(&USB_SDIO_InitStructure);
  ...
  SDIO->CLKCR|=1<<8;			//SDIOCK使能  
 	...
 	for(i=0;i<74;i++)
	{
		...
		USB_SDIO_CmdInitStructure.SDIO_Argument = 0x0;//发送CMD0进入IDLE STAGE模式命令.
    USB_SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_GO_IDLE_STATE; //cmd0
    USB_SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_No;  //无响应
    USB_SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
    USB_SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;  //则CPSM在开始发送命令之前等待数据传输结束。 
    SDIO_SendCommand(&USB_SDIO_CmdInitStructure);	  		//写命令进命令寄存器
  ...
  USB_SDIO_CmdInitStructure.SDIO_Argument = SD_CHECK_PATTERN;	//发送CMD8,短响应,检查SD卡接口特性
  USB_SDIO_CmdInitStructure.SDIO_CmdIndex = SDIO_SEND_IF_COND;	//cmd8
  USB_SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;	 //r7
  USB_SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;			 //关闭等待中断
  USB_SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
  SDIO_SendCommand(&USB_SDIO_CmdInitStructure);
  ...
  errorstatus=CmdResp7Error();						//等待R7响应
 	...
 	if(errorstatus==SD_OK) 								//R7响应正常
	{
	  ...
	  USB_SDIO_CmdInitStructure.SDIO_Argument = 0x00;//发送CMD55,短响应	
    USB_SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_APP_CMD;
    USB_SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;
    USB_SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
    USB_SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
    SDIO_SendCommand(&USB_SDIO_CmdInitStructure);		//发送CMD55,短响应	 
	...
	{																  
		...
		{	   										   
		  USB_SDIO_CmdInitStructure.SDIO_Argument = 0x00;//发送CMD55,短响应
      USB_SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_APP_CMD;	  //CMD55
      USB_SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;
      USB_SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
      USB_SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
      SDIO_SendCommand(&USB_SDIO_CmdInitStructure);			//发送CMD55,短响应	 
      ...
      USB_SDIO_CmdInitStructure.SDIO_Argument = SD_VOLTAGE_WINDOW_SD | SDType;	//发送ACMD41,短响应	
      USB_SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_SD_APP_OP_COND;
      USB_SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;  //r3
      USB_SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
      USB_SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
      SDIO_SendCommand(&USB_SDIO_CmdInitStructure);
 	...
 	}else//MMC卡
	{
		...
		while((!validvoltage)&&(count<SD_MAX_VOLT_TRIAL))
		{	   										   				   
			USB_SDIO_CmdInitStructure.SDIO_Argument = SD_VOLTAGE_WINDOW_MMC;//发送CMD1,短响应	   
      USB_SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_SEND_OP_COND;
      USB_SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;  //r3
      USB_SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
      USB_SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
      SDIO_SendCommand(&USB_SDIO_CmdInitStructure);
...
static SD_Error SD_InitializeCards(void)
{
  ...
  if (SDIO_GetPowerState() == SDIO_PowerState_OFF)	//检查电源状态,确保为上电状态
  {
	...
	{
		USB_SDIO_CmdInitStructure.SDIO_Argument = 0x0;//发送CMD2,取得CID,长响应
    USB_SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_ALL_SEND_CID;
    USB_SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Long;
    USB_SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
    USB_SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
    SDIO_SendCommand(&USB_SDIO_CmdInitStructure);//发送CMD2,取得CID,长响应	
	...
	{
		USB_SDIO_CmdInitStructure.SDIO_Argument = 0x00;//发送CMD3,短响应 
    USB_SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_SET_REL_ADDR;	//cmd3
    USB_SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short; //r6
    USB_SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
    USB_SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
    SDIO_SendCommand(&USB_SDIO_CmdInitStructure);	//发送CMD3,短响应 
    ...
    if (SDIO_MULTIMEDIA_CARD==CardType)
    {
		  ...
		  USB_SDIO_CmdInitStructure.SDIO_Argument = (u32)(rca<<16);//发送CMD3,短响应 
      USB_SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_SET_REL_ADDR;	//cmd3
      USB_SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short; //r6
      USB_SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
      USB_SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
    ...
    USB_SDIO_CmdInitStructure.SDIO_Argument = (uint32_t)(rca << 16);//发送CMD9+卡RCA,取得CSD,长响应 
    USB_SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_SEND_CSD;
    USB_SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Long;
    USB_SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
    USB_SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
    SDIO_SendCommand(&USB_SDIO_CmdInitStructure);
...
SD_Error SD_SelectDeselect(u32 addr)
{
  ...
  USB_SDIO_CmdInitStructure.SDIO_Argument =  addr;//发送CMD7,选择卡,短响应	
  USB_SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_SEL_DESEL_CARD;
  USB_SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;
  USB_SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
  USB_SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
  SDIO_SendCommand(&USB_SDIO_CmdInitStructure);//发送CMD7,选择卡,短响应
...
{	  
  ...
  SDIO->DCTRL=0x0;	//数据控制寄存器清零(关DMA) 
	...
	if(CardType==SDIO_HIGH_CAPACITY_SD_CARD)//大容量卡
	{
  	...
  	USB_SDIO_DataInitStructure.SDIO_DataBlockSize= SDIO_DataBlockSize_1b ;//清除DPSM状态机配置
	  USB_SDIO_DataInitStructure.SDIO_DataLength= 0 ;
	  USB_SDIO_DataInitStructure.SDIO_DataTimeOut=SD_DATATIMEOUT ;
	  USB_SDIO_DataInitStructure.SDIO_DPSM=SDIO_DPSM_Enable;
	  USB_SDIO_DataInitStructure.SDIO_TransferDir=SDIO_TransferDir_ToCard;
	  USB_SDIO_DataInitStructure.SDIO_TransferMode=SDIO_TransferMode_Block;
    SDIO_DataConfig(&USB_SDIO_DataInitStructure);
	...
	if((blksize>0)&&(blksize<=2048)&&((blksize&(blksize-1))==0))
	{
		...
		USB_SDIO_CmdInitStructure.SDIO_Argument =  blksize;
    USB_SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_SET_BLOCKLEN;
    USB_SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;
    USB_SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
    USB_SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
    SDIO_SendCommand(&USB_SDIO_CmdInitStructure);//发送CMD16+设置数据长度为blksize,短响应
	  ...
	  USB_SDIO_DataInitStructure.SDIO_DataBlockSize= power<<4 ;//清除DPSM状态机配置
	  USB_SDIO_DataInitStructure.SDIO_DataLength= blksize ;
	  USB_SDIO_DataInitStructure.SDIO_DataTimeOut=SD_DATATIMEOUT ;
	  USB_SDIO_DataInitStructure.SDIO_DPSM=SDIO_DPSM_Enable;
	  USB_SDIO_DataInitStructure.SDIO_TransferDir=SDIO_TransferDir_ToSDIO;
	  USB_SDIO_DataInitStructure.SDIO_TransferMode=SDIO_TransferMode_Block;
    SDIO_DataConfig(&USB_SDIO_DataInitStructure);
	
	  USB_SDIO_CmdInitStructure.SDIO_Argument =  addr;
    USB_SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_READ_SINGLE_BLOCK;
    USB_SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;
    USB_SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
    USB_SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
    SDIO_SendCommand(&USB_SDIO_CmdInitStructure);//发送CMD17+从addr地址出读取数据,短响应 
...
static SD_Error SD_ReadMultiBlocks(u8 *buf,long long addr,u16 blksize,u32 nblks)
{
  ...
  SDIO->DCTRL=0x0;		//数据控制寄存器清零(关DMA)   
	if(CardType==SDIO_HIGH_CAPACITY_SD_CARD)//大容量卡
	{
	  ...
	  USB_SDIO_DataInitStructure.SDIO_DataBlockSize= 0; ;//清除DPSM状态机配置
	  USB_SDIO_DataInitStructure.SDIO_DataLength= 0 ;
	  USB_SDIO_DataInitStructure.SDIO_DataTimeOut=SD_DATATIMEOUT ;
	  USB_SDIO_DataInitStructure.SDIO_DPSM=SDIO_DPSM_Enable;
	  USB_SDIO_DataInitStructure.SDIO_TransferDir=SDIO_TransferDir_ToCard;
	...
	if((blksize>0)&&(blksize<=2048)&&((blksize&(blksize-1))==0))
	{
	  ...
	  USB_SDIO_CmdInitStructure.SDIO_Argument =  blksize;//发送CMD16+设置数据长度为blksize,短响应 
		USB_SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_SET_BLOCKLEN;
		USB_SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;
		USB_SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
		USB_SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
	...
	{									    
		   ...
		   USB_SDIO_DataInitStructure.SDIO_DataBlockSize= power<<4; ;//nblks*blksize,512块大小,卡到控制器
			 USB_SDIO_DataInitStructure.SDIO_DataLength= nblks*blksize ;
			 USB_SDIO_DataInitStructure.SDIO_DataTimeOut=SD_DATATIMEOUT ;
			 USB_SDIO_DataInitStructure.SDIO_DPSM=SDIO_DPSM_Enable;
			 USB_SDIO_DataInitStructure.SDIO_TransferDir=SDIO_TransferDir_ToSDIO;
			 USB_SDIO_DataInitStructure.SDIO_TransferMode=SDIO_TransferMode_Block;
       ...
       USB_SDIO_CmdInitStructure.SDIO_Argument =  addr;//发送CMD18+从addr地址出读取数据,短响应 
	     USB_SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_READ_MULT_BLOCK;
		   USB_SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;
		   USB_SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
		   USB_SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
		   SDIO_SendCommand(&USB_SDIO_CmdInitStructure);	
		...
		while(SDIO_GetFlagStatus(SDIO_FLAG_RXDAVL) != RESET)	//FIFO里面,还存在可用数据
		{
			*tempbuff=SDIO->FIFO;	//循环读取数据
	 		...
	 		if(SDIO_GetFlagStatus(SDIO_FLAG_DATAEND) != RESET)		//接收结束
			{
				...
				{				
					USB_SDIO_CmdInitStructure.SDIO_Argument =  0;//发送CMD12+结束传输
				  USB_SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_STOP_TRANSMISSION;
					USB_SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;
					USB_SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
					USB_SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
					SDIO_SendCommand(&USB_SDIO_CmdInitStructure);	
  ...
  SDIO_DataConfig(&USB_SDIO_DataInitStructure);
	...
	if((blksize>0)&&(blksize<=2048)&&((blksize&(blksize-1))==0))
	{
		...
		if(errorstatus!=SD_OK)return errorstatus;   	//响应错误	 
			...
			USB_SDIO_CmdInitStructure.SDIO_Argument = (u32)RCA<<16;//发送CMD13,查询卡的状态,短响应 	
		  USB_SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_SEND_STATUS;
			USB_SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;
			USB_SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
			USB_SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
			SDIO_SendCommand(&USB_SDIO_CmdInitStructure);	
	...
	if(timeout==0)return SD_ERROR;
			...
			USB_SDIO_CmdInitStructure.SDIO_Argument = addr;//发送CMD24,写单块指令,短响应 	
			USB_SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_WRITE_SINGLE_BLOCK;
			USB_SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;
			USB_SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
			USB_SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
			SDIO_SendCommand(&USB_SDIO_CmdInitStructure);	
	...
	USB_SDIO_DataInitStructure.SDIO_DataBlockSize= power<<4; ;	//blksize, 控制器到卡	
	USB_SDIO_DataInitStructure.SDIO_DataLength= blksize ;
	USB_SDIO_DataInitStructure.SDIO_DataTimeOut=SD_DATATIMEOUT ;
	USB_SDIO_DataInitStructure.SDIO_DPSM=SDIO_DPSM_Enable;
	USB_SDIO_DataInitStructure.SDIO_TransferDir=SDIO_TransferDir_ToCard;
...
//		INTX_ENABLE();//开启总中断
	...
	}else if(DeviceMode==SD_DMA_MODE)
	{
		...
		if(timeout==0)
		{
  		USB_SD_Init();	 					//重新初始化SD卡,可以解决写入死机的问题
...
static SD_Error SD_WriteMultiBlocks(u8 *buf,long long addr,u16 blksize,u32 nblks)
{
  ...
  SDIO_DataConfig(&USB_SDIO_DataInitStructure);
	...
	{					  
    	...
    	{
			//提高性能
				USB_SDIO_CmdInitStructure.SDIO_Argument = (u32)RCA<<16;		//发送ACMD55,短响应 	
				USB_SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_APP_CMD;
				USB_SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;
				USB_SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
				USB_SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
			...
			if(errorstatus!=SD_OK)return errorstatus;				 
				...
				USB_SDIO_CmdInitStructure.SDIO_Argument =nblks;		//发送CMD23,设置块数量,短响应 	 
				USB_SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_SET_BLOCK_COUNT;
				USB_SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;
				USB_SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
				USB_SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
				SDIO_SendCommand(&USB_SDIO_CmdInitStructure);
			...
			if(errorstatus!=SD_OK)return errorstatus;		
				...
				USB_SDIO_CmdInitStructure.SDIO_Argument =addr;	//发送CMD25,多块写指令,短响应 	  
				USB_SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_WRITE_MULT_BLOCK;
				USB_SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;
				USB_SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
				USB_SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
				SDIO_SendCommand(&USB_SDIO_CmdInitStructure);	
...
//处理SDIO传输过程中的各种中断事务
//返回值:错误代码
SD_Error USB_SD_ProcessIRQSrc(void)
{
...
SD_Error SDEnWideBus(u8 enx)
{
	...
	{
		  USB_SDIO_CmdInitStructure.SDIO_Argument = (uint32_t) RCA << 16;//发送CMD55+RCA,短响应	
      USB_SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_APP_CMD;
      USB_SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;
      USB_SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
      USB_SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
      SDIO_SendCommand(&USB_SDIO_CmdInitStructure);
	 	...
	 	if(errorstatus!=SD_OK)return errorstatus; 
		  ...
		  USB_SDIO_CmdInitStructure.SDIO_Argument = arg;//发送ACMD6,短响应,参数:10,4位;00,1位.	
      USB_SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_APP_SD_SET_BUSWIDTH;
      USB_SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;
      USB_SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
      USB_SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
      SDIO_SendCommand(&USB_SDIO_CmdInitStructure);
...
SD_Error IsCardProgramming(u8 *pstatus)
{
  ...
  USB_SDIO_CmdInitStructure.SDIO_Argument = (uint32_t) RCA << 16; //卡相对地址参数
  USB_SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_SEND_STATUS;//发送CMD13 	
  USB_SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;
  USB_SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
  USB_SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
  SDIO_SendCommand(&USB_SDIO_CmdInitStructure);	
...
//	}
//	
//	USB_SDIO_CmdInitStructure.SDIO_Argument = (uint32_t) RCA << 16;//发送CMD13,短响应		 
//  USB_SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_SEND_STATUS;
//  USB_SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;
//  USB_SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
//  USB_SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
...
{ 
	...
	USB_SDIO_CmdInitStructure.SDIO_Argument = (uint32_t)8;	 //发送CMD16,短响应,设置Block Size为8字节	
  USB_SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_SET_BLOCKLEN; //	 cmd16
  USB_SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;  //r1
  USB_SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
  USB_SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
  SDIO_SendCommand(&USB_SDIO_CmdInitStructure);
  ...
  USB_SDIO_CmdInitStructure.SDIO_Argument = (uint32_t) RCA << 16; 
  USB_SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_APP_CMD;//发送CMD55,短响应 	
  USB_SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;
  USB_SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
  USB_SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
  SDIO_SendCommand(&USB_SDIO_CmdInitStructure);
  ...
  USB_SDIO_DataInitStructure.SDIO_DataTimeOut = SD_DATATIMEOUT;
  USB_SDIO_DataInitStructure.SDIO_DataLength = 8;  //8个字节长度,block为8字节,SD卡到SDIO.
  USB_SDIO_DataInitStructure.SDIO_DataBlockSize = SDIO_DataBlockSize_8b  ;  //块大小8byte 
  USB_SDIO_DataInitStructure.SDIO_TransferDir = SDIO_TransferDir_ToSDIO;
  USB_SDIO_DataInitStructure.SDIO_TransferMode = SDIO_TransferMode_Block;
  USB_SDIO_DataInitStructure.SDIO_DPSM = SDIO_DPSM_Enable;
  SDIO_DataConfig(&USB_SDIO_DataInitStructure);		

  USB_SDIO_CmdInitStructure.SDIO_Argument = 0x0;
  USB_SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_SD_APP_SEND_SCR;	//发送ACMD51,短响应,参数为0	
  USB_SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;  //r1
  USB_SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
  USB_SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
  SDIO_SendCommand(&USB_SDIO_CmdInitStructure);
